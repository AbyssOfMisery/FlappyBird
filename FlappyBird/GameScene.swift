...
//        self.addChild(bgmPlayer)
    ...
    func createPuase() {
        let pauseBtn = SKSpriteNode(imageNamed: "flappybird_19")
        pauseBtn.name = "pauseBtn"
        pauseBtn.position = CGPoint(x: self.size.width / 1.2, y: self.size.height * 0.9)
        pauseBtn.setScale(2)
        pauseBtn.zPosition = Layer.hud
        self.addChild(pauseBtn)
    }
    
    func createScore() {
        scoreLabel = SKLabelNode(fontNamed: "OpenSans-Italic")
...
//        self.addChild(ceiling)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        case .playing:
            ...
            if let location = touch?.location(in: self) {
                     ...
                     if nodesArray.first?.name == "pauseBtn" {
    ...
    func recordBestScore() {
        ...
        if self.score > bestScore {
            ...
            userDefaults.set(currentScore, forKey: "score")
        }
...
extension GameScene: GKGameCenterControllerDelegate {
    ...
    func showLeaderborad() {
        if GKLocalPlayer.local.isAuthenticated {
            ...
            gameCenter.gameCenterDelegate = self as GKGameCenterControllerDelegate
    ...
    func updateLeaderboard() {
        if GKLocalPlayer.local.isAuthenticated {
            let leaderboradIdentifier = "FlappyBirdLikeBestScore"
    ...
    func checkForAchivement() {
        if GKLocalPlayer.local.isAuthenticated {
            ...
            if inGameBestScore >= 100 {
                let identifier = "FlappyBirdLikePlatinumMedal"
                ...
                let identifier = "FlappyBirdLikeGoldMedal"
                let achive = GKAchievement(identifier: identifier)
                achive.showsCompletionBanner = true
                achive.percentComplete = 100.0
                GKAchievement.report([achive], withCompletionHandler: nil)
                ...
                let identifier = "FlappyBirdLikeSilverMedal"
                let achive = GKAchievement(identifier: identifier)
                achive.showsCompletionBanner = true
                achive.percentComplete = 100.0
                GKAchievement.report([achive], withCompletionHandler: nil)
                ...
                let identifier = "FlappyBirdLikeBronze"
                let achive = GKAchievement(identifier: identifier)
                achive.showsCompletionBanner = true
                achive.percentComplete = 100.0
                GKAchievement.report([achive], withCompletionHandler: nil)
